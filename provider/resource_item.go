...
func resourceCreateItem(d *schema.ResourceData, m interface{}) error {
	...
	for i, tfTags := range tfTags {
		tags[i] = tfTags.(string)
	}
...
func resourceReadItem(d *schema.ResourceData,  m interface{}) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), "not found") {
			...
			return fmt.Errorf("error finding Item with ID %", itemId)
		}
	}
...
func resourceDeleteItem(d *schema.ResourceData, m interface{}) error {
 ...
 func resourceExistsItem(d *schema.ResourceItem, m interface{}) error {
	apiClient := m.(*client.Client)
